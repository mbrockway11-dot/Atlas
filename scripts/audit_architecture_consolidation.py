"""Audit Atlas Architecture Consolidation v1 invariants."""
...
from atlas.investment.artifacts import validate_registry
...
def audit_loader(path: Path) -> list[str]:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.ImportFrom):
            if node.module == FORBIDDEN_IMPORT:
...
def main() -> int:
    ...
    print(f"Registered artifacts: {len(__import__('atlas.investment.artifacts', fromlist=['ARTIFACTS']).ARTIFACTS)}")
    print(f"Registered jobs: {len(JOBS)}")
    print(f"Audited loaders: {len(ACTIVE_LOADERS)}")
    return 0
