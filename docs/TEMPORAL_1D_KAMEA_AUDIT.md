...
## Frozen conclusions — control architecture

Durable, and not to be relitigated by a downstream result:

> `window_uniform` is an **inadequate** R1 control generator.
>
> `stratified_era` is the **canonical** R1 temporal-control generator.
>
> `symmetric_displacement` is an **independently validated alternative**.
>
> `representation_matched` is a **diagnostic upper bound**, not a canonical
> analysis control, because it conditions directly on a feature under study.

And the stronger result, which is what moves translation masking from a
theoretical hazard to an observed failure mode:

> **Upstream certification is empirically necessary.** A cohort generated by
> an inadequate control procedure exhibited B3D event/control separability
> that translation-normalized B2 concealed.

### On reading `p = 1.000`

The canonical generator's "accuracy 0.500, p = 1.000" is a good diagnostic
outcome and is *not* the acceptance claim. Chance accuracy can equally arise
from an empty or saturated representation. Acceptance rests on the **combined
gate**: no practically meaningful event/control predictability, sufficient
support overlap, acceptable unseen-class rate, no masking, no degeneracy
imbalance. The verdict is the conjunction, never the p-value.
